  ...
  <div v-if="product" class="page-section">
    <div class="container">
      <el-breadcrumb separator="/" style="margin-bottom: 28px">
        <el-breadcrumb-item :to="{ path: '/' }">{{ t('detail_crumb_home') }}</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/products' }">{{ t('detail_crumb_products') }}</el-breadcrumb-item>
      ...
      <div class="detail">
        ...
        <div class="info">
          ...
          <div class="features" v-if="product.features && product.features.length">
            <div class="f-title">{{ t('detail_features') }}</div>
          ...
          <div class="actions">
            <el-button type="primary" size="large" round @click="goContact">{{ t('detail_quote') }}</el-button>
            <el-button size="large" round @click="goContact">{{ t('detail_wa') }}</el-button>
          </div>
        </div>
      </div>
      ...
      <!-- 相关产品 -->
      <div class="related">
        <h3 class="rl-title">{{ t('detail_related') }}</h3>
        <div class="grid">
...
const categoryLabel = computed(() => (product.value?.category ? t('catL_' + product.value.category) : ''))
...
.gallery { background: #f5f7fa; border-radius: 12px; overflow: hidden; }
.gallery img { width: 100%; display: block; }

.info { padding-top: 4px; }
.tag { display: inline-block; font-size: 13px; color: var(--yl-primary); background: rgba(64,158,255,.10); padding: 3px 12px; border-radius: 20px; margin-bottom: 14px; }
...
.sp-value { display: inline-block; background: #ecf5ff; color: var(--yl-primary); padding: 6px 14px; border-radius: 8px; font-size: 14px; }
...
/* 商品详情：通栏富文本 */
.detail-section { margin-top: 56px; background: #fff; border: 1px solid #e4e7ed; border-radius: 8px; padding: 28px 32px; }
.ds-title { font-size: 20px; margin: 0 0 18px; padding-bottom: 14px; border-bottom: 1px solid #e4e7ed; color: var(--yl-text); }
...
.rich :deep(th) { background: #ecf5ff; color: var(--yl-primary); font-weight: 600; }
