...
- 模型配置（全局弹框）：对话提供方（OpenAI 兼容 / Anthropic）、向量模型、温度与检索参数，用户级保存
- 使用统计（全局弹框）：KPI 总览 + 近 14 天提问趋势（ECharts）+ 热门问题 Top10
- 知识图谱（全局弹框）：基于 three.js 的 3D 文档关系可视化，拖拽/缩放/聚焦
...
| `/login` | 登录 | 公开 |
| `/register` | 注册 | 公开 |
| `/home` | 主页 | 需登录；由 AppHeader + SidebarPanel + ChatPanel 组装 |
| `/` | 重定向 | 跳转 `/home` |

> 模型配置 / 使用统计 / 知识图谱 通过头像下拉以全局弹框打开（`stores/ui.ts` + `components/GlobalDialogs.vue`），不走路由。
...
├── App.vue                 # 根组件：路由出口 + 全局弹框宿主
├── api/                    # 接口封装：auth / chat / request / settings / stats / upload
├── components/             # AppHeader / SidebarPanel / ChatPanel / GlobalDialogs
├── router/index.ts         # 路由与登录态守卫
├── stores/                 # Pinia：user（登录态）/ chat（工作台状态）/ ui（全局弹框）
